...
namespace sop {
        ...
        : m_TicksPerSecond(ticksPerSecond), m_MaxFrameDelta(maxFrameDelta)
    {
        ...
        SOP_ASSERT(m_MaxFrameDelta > Duration::zero(), "FixedStepScheduler requires a positive max frame delta");
    }
    ...
    uint32_t FixedStepScheduler::Advance(Duration elapsed)
    {
        ...
        const uint32_t ticksDue = static_cast<uint32_t>(std::floor(m_AccumulatedTicks + kRoundingEpsilon));
