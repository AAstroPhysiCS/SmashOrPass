...
namespace sop {
    ...
    ImageNode UIBuilder::Image(uint64_t textureId) {
        return ImageNode{m_Screen,
                         m_Screen.Create(WidgetKind::Image, ImageData{textureId})};
    }

    LabelNode UIBuilder::Label(std::string text) {
        return LabelNode{m_Screen,
                         m_Screen.Create(WidgetKind::Label, LabelData{std::move(text), {}})};
    }

    ButtonNode UIBuilder::Button(std::string text) {
        return ButtonNode{m_Screen,
                          m_Screen.Create(WidgetKind::Button, ButtonData{std::move(text), {}, {}})};
    }
}  // namespace sop