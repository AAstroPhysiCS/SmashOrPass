...
void ScoreboardScreen::Build(UIBuilder& builder) {
    ...
    const OverallMatchupStats& p1VsAi = GetAppCtx().overallStats.StatsFor(MatchupType::Player1VsAi);
    ...
    auto descriptorColumn = builder.Column().Spacing(8.0f).Add(
        ...
        builder.Label("Opponent Wins").Font(FontId::Small).TextColor(Theme::TEXT_PRIMARY_COLOR),
        builder.Label("P1 Win Rate").Font(FontId::Small).TextColor(Theme::TEXT_PRIMARY_COLOR),
        builder.Label("P1 Damage").Font(FontId::Small).TextColor(Theme::TEXT_PRIMARY_COLOR),
        builder.Label("P1 Hits").Font(FontId::Small).TextColor(Theme::TEXT_PRIMARY_COLOR),
        builder.Label("Opponent Damage").Font(FontId::Small).TextColor(Theme::TEXT_PRIMARY_COLOR),
        builder.Label("Opponent Hits").Font(FontId::Small).TextColor(Theme::TEXT_PRIMARY_COLOR));

    const auto makeMatchupColumn = [&](const char* title, const OverallMatchupStats& stats) {
        return builder.Column().Spacing(8.0f).Add(
            builder.Label(title)
                .Font(FontId::Small)
    ...
    auto content =
        builder.Column()
            .Spacing(18.0f)
            .Align(Alignment::TopCenter)
            .Add(builder.Label("SCOREBOARD").Font(FontId::Title).Align(Alignment::TopCenter),
                 table,
                 builder.Button("Back")
                     .Align(Alignment::TopCenter)
