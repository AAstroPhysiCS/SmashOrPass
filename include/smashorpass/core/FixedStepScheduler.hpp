...
class FixedStepScheduler final {
   public:
    ...
    explicit FixedStepScheduler(uint32_t ticksPerSecond,
                                Duration maxFrameDelta = std::chrono::milliseconds(250));
    ...
    [[nodiscard]] uint32_t GetTicksPerSecond() const {
        return m_TicksPerSecond;
    }
    [[nodiscard]] Duration GetMaxFrameDelta() const {
