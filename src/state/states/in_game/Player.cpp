...
[[nodiscard]] bool IsActionHeld(const InputHelper& input,
...
std::optional<SDL_FRect> Player::GetBaselineCollisionBox(AppCtx& ctx) const {
    auto asset = ctx.m_Assets.Get(m_Asset);
    if (!asset) {
        return std::nullopt;
    }
...
void Player::TickGameLogic(AppCtx& ctx, const Arena& arena) {
    ...
    const bool moveLeft =
        IsActionHeld(ctx.m_InputHelper, m_InputTranslationHelper, InputAction::MOVE_LEFT);
    const bool moveRight =
        IsActionHeld(ctx.m_InputHelper, m_InputTranslationHelper, InputAction::MOVE_RIGHT);
    const bool attackHeld =
        IsActionHeld(ctx.m_InputHelper, m_InputTranslationHelper, InputAction::ATTACK);
    ...
    // Collision handling
    auto arenaAsset = ctx.m_Assets.Get(arena.asset);
    if (!arenaAsset) {
        return;
    }
...
Result<void> Player::Render(AppCtx& ctx, const Arena& arena) const {
    TRY(asset, ctx.m_Assets.Get(m_Asset));
    ...
    return ctx.m_Renderer.DrawTexture(sheet->second.m_Texture.get(), params);
}
...
Result<void> Player::RenderCollisionBox(AppCtx& ctx, const Arena& arena) const {
    ...
    return ctx.m_Renderer.DrawRect(MapBaselineRectToArena(*collisionBox, arena.dimensions),
                                   Color{255, 230, 0, 255});
}
