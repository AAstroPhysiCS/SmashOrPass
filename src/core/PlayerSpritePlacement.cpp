...
Vec2 LocalFramePointToWorld(const PlayerCharacterState& player,
    ...
    const Vec2 origin{
        static_cast<float>(frame.anchor_x) * scale,
        static_cast<float>(frame.anchor_y) * scale,
    };

    const float destinationX = facingRight ? player.Position.x - (destinationWidth - origin.x)
                                           : player.Position.x - origin.x;

    const float destinationY = player.Position.y - origin.y;

    if (facingRight) {
        localPoint.x = frameWidth - localPoint.x;
    }

    return Vec2{
        destinationX + localPoint.x * scale,
        destinationY + localPoint.y * scale,
    };
}
